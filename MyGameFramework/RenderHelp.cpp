...
namespace renderHelp
{
    struct WICInitializer
    {
        ...
        void CleanupBitmapInfos() {

            // 순회를 시작할 수 있는 BitmapInfo의 포인터가 있어야 해요.
            BitmapInfo* current = m_LinkStart;
            while (current != nullptr) {
                BitmapInfo* next = current->m_pPrevLink; // 다음 링크 저장
                std::cout << current->GetWidth() << std::endl;
                delete current; // private 소멸자는 friend 관계로 호출 가능
                current = next;
            }
            m_LinkStart = nullptr;
            m_LinkEnd = nullptr;
        ...
        BitmapInfo* CreateBitmapInfo(HBITMAP hBitmap)
        {
            ...
            // 생성된 비트맵 노드를 관리해야 한다.
            // 노드를 링크 시켜서 관리해 봅시다.
            BitmapInfo* pNewBitmap = DBG_NEW BitmapInfo(hBitmap);
